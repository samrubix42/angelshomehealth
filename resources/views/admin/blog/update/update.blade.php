<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="flex items-center justify-between gap-4 pb-2 border-b border-zinc-200/80">
        <div class="space-y-1">
            <div class="flex items-center gap-2">
                <a href="/admin/blogs" class="text-xs text-zinc-500 hover:text-zinc-900 inline-flex items-center gap-1 font-medium transition-colors">
                    <i class="ri-arrow-left-line"></i>
                    <span>Back to Blogs</span>
                </a>
                <span class="text-xs text-zinc-300">•</span>
                <span class="text-xs text-zinc-500 font-medium">Edit Article #{{ $blogId }}</span>
            </div>
        ...
        <div class="flex items-center gap-2">
            <a href="/admin/blogs" class="px-4 py-2 rounded-lg border border-zinc-200 bg-white text-zinc-900 font-medium text-xs hover:bg-zinc-100 transition-colors shadow-xs">
                Cancel
            </a>
            <button type="submit" form="blog-update-form" class="px-5 py-2 rounded-lg bg-zinc-900 text-zinc-50 font-medium text-xs hover:bg-zinc-800 transition-all shadow-xs inline-flex items-center gap-1.5 cursor-pointer">
    ...
    <!-- 2. MAIN FORM CARD -->
    <form id="blog-update-form" wire:submit.prevent="save" class="space-y-6">
        
        <!-- GENERAL ARTICLE INFO CARD -->
        <div class="bg-white rounded-xl border border-zinc-200 shadow-xs p-6 space-y-5">
            <h2 class="text-sm font-semibold text-zinc-900 border-b border-zinc-100 pb-3 flex items-center justify-between">
                <div class="flex items-center gap-2">
                    <i class="ri-edit-line text-zinc-400"></i>
                    <span>Article Details</span>
                </div>
                <a href="/blog/{{ $slug }}" target="_blank" class="text-xs text-zinc-500 hover:text-zinc-900 inline-flex items-center gap-1 font-medium">
                    <i class="ri-external-link-line"></i> View Live Article
                </a>
            </h2>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <!-- Article Title -->
                <div>
                    <label class="block text-xs font-medium text-zinc-700 mb-1">Article Title *</label>
                    <input type="text" 
                           wire:model="title" 
                           placeholder="e.g. 5 Essential Tips for Post-Surgery Care at Home" 
                ...
                <div>
                    ...
                    <select wire:model="category_id" class="w-full bg-white border border-zinc-200 focus:border-zinc-400 focus:outline-none focus:ring-2 focus:ring-zinc-950/10 rounded-lg px-3 py-2 text-xs text-zinc-900 transition-all">
                        <option value="">Select a Category</option>
                        @foreach($this->categories as $category)
                            <option value="{{ $category->id }}">{{ $category->title }}</option>
                        @endforeach
                    </select>
                    @error('category_id') <span class="text-[11px] text-red-500 mt-1 block">{{ $message }}</span> @enderror
                </div>
            </div>
            ...
            <div>
                ...
                <div class="relative">
                    <span class="absolute left-3 top-2 text-xs font-mono text-zinc-400">/blog/</span>
                    <input type="text" 
                           wire:model="slug" 
                           placeholder="5-essential-tips-for-post-surgery-care" 
                           class="w-full bg-white border border-zinc-200 focus:border-zinc-400 focus:outline-none focus:ring-2 focus:ring-zinc-950/10 rounded-lg pl-16 pr-3 py-2 text-xs font-mono text-zinc-900 placeholder:text-zinc-400 transition-all">
                </div>
            ...
            <div>
                ...
                <textarea wire:model="short_description" 
                          rows="2" 
                          placeholder="Brief summary snippet displayed on blog listing cards..." 
            ...
            <div>
                <label class="block text-xs font-medium text-zinc-700 mb-1">Full Article Content *</label>
                <div wire:ignore
                     x-data="{
            ...
            <div class="pt-2">
                <label class="flex items-center gap-2.5 cursor-pointer select-none">
                    ...
                    <span class="text-xs font-medium text-zinc-700">Publish article immediately on website</span>
                </label>
            </div>
        </div>
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <input type="text" 
                           ...
                           placeholder="5 Essential Tips for Post-Surgery Care | Angels Home Health" 
                           class="w-full bg-white border border-zinc-200 focus:border-zinc-400 focus:outline-none focus:ring-2 focus:ring-zinc-950/10 rounded-lg px-3 py-2 text-xs text-zinc-900 placeholder:text-zinc-400 transition-all">
                ...
                <div>
                    ...
                    <textarea wire:model="meta_description" 
                              rows="2" 
                              placeholder="Discover vital post-surgery home care guidelines provided by skilled nurses..." 
                              class="w-full bg-white border border-zinc-200 focus:border-zinc-400 focus:outline-none focus:ring-2 focus:ring-zinc-950/10 rounded-lg px-3 py-2 text-xs text-zinc-900 placeholder:text-zinc-400 transition-all"></textarea>
                ...
                <div>
                    ...
                    <input type="text" 
                           ...
                           placeholder="post surgery care, home health nursing, recovery tips florida" 
                           class="w-full bg-white border border-zinc-200 focus:border-zinc-400 focus:outline-none focus:ring-2 focus:ring-zinc-950/10 rounded-lg px-3 py-2 text-xs text-zinc-900 placeholder:text-zinc-400 transition-all">
        ...
        <div class="flex items-center justify-end gap-3 pt-2">
            <a href="/admin/blogs" class="px-5 py-2.5 rounded-lg border border-zinc-200 bg-white text-zinc-900 font-medium text-xs hover:bg-zinc-100 transition-colors shadow-xs">
                Cancel
            </a>
            <button type="submit" class="px-6 py-2.5 rounded-lg bg-zinc-900 text-zinc-50 font-medium text-xs hover:bg-zinc-800 transition-all shadow-xs inline-flex items-center gap-2 cursor-pointer">
